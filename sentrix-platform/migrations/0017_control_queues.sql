-- Global machine workers must not scan tenant RLS tables. These private queues
-- contain only routing metadata. After claiming one row, SECURITY DEFINER code
-- sets app.current_org transaction-locally before touching tenant data.

CREATE TABLE build_control_queue (
    build_id uuid PRIMARY KEY,
    org_id uuid NOT NULL,
    status text NOT NULL CHECK (
        status IN ('queued','building','scanning','succeeded','rejected','failed')
    ),
...
CREATE TABLE deployment_control_queue (
    deployment_id uuid PRIMARY KEY,
    org_id uuid NOT NULL,
    status text NOT NULL CHECK (
        status IN ('pending','running','succeeded','rolled_back','failed','cancelled')
    ),
    lease_worker_id uuid,
    lease_expires_at timestamptz,
...
CREATE INDEX build_control_claim_idx
    ON build_control_queue (status, lease_expires_at, created_at);
CREATE INDEX deployment_control_claim_idx
    ON deployment_control_queue (status, lease_expires_at, created_at);

REVOKE ALL ON build_control_queue, deployment_control_queue FROM PUBLIC;
REVOKE ALL ON build_control_queue, deployment_control_queue FROM sentrix_app;
...
AFTER INSERT OR UPDATE OF status, lease_worker_id, lease_expires_at
ON builds
...
BEGIN
    IF NEW.status IN ('pending','running') THEN
        INSERT INTO public.deployment_control_queue (deployment_id, org_id, status)
        VALUES (NEW.id, NEW.org_id, NEW.status)
...
BEGIN
    ...
    IF p_outcome <> 'succeeded' THEN
        UPDATE public.builds
           SET status = p_outcome,
               ...
               updated_at = now()
         WHERE id = p_build_id AND org_id = v_org_id;
    ...
    SELECT e.auto_deploy, e.runtime_mode, e.secret_provider, bot.library
    ...
    v_config_hash := encode(
        sha256(convert_to(concat_ws(':', v_runtime_mode, v_secret_provider, v_library), 'UTF8')),
        'hex'
    );
    ...
    v_identity_key := encode(
        sha256(
            convert_to(
                ...
                || ':' || v_config_hash || ':' || v_secret_version::text,
                'UTF8'
            )
        ),
        'hex'
    ...
    INSERT INTO public.releases (
        ...
        p_release_id, v_org_id, v_build.environment_id, v_build.id,
        p_image_digest, p_image_ref, v_config_hash, v_secret_version, v_identity_key
    )
...
BEGIN
    ...
    IF v_dep.id IS NULL OR v_dep.status NOT IN ('pending','running') THEN
        DELETE FROM public.deployment_control_queue WHERE deployment_id = v_dep_id;
        RETURN;
    END IF;
    ...
    IF v_dep.step = 'test' THEN
        ...
        IF v_image_ref IS NULL THEN
            UPDATE public.deployments
               SET status = 'failed', error = 'release has no immutable image_ref', updated_at = now()
             WHERE id = v_dep.id;
            RETURN QUERY
            ...
            RETURN;
        END IF;
        UPDATE public.deployments SET step = 'prewarm', error = NULL, updated_at = now()
         WHERE id = v_dep.id;
    ...
    IF v_dep.step = 'prewarm' THEN
        SELECT e.cell_id INTO v_cell_id
          FROM public.environments e
         WHERE e.id = v_dep.environment_id AND e.org_id = v_org_id AND e.status = 'active';
        SELECT n.id INTO v_node_id
          FROM public.nodes n
         WHERE n.cell_id = v_cell_id AND n.status = 'active'
         ORDER BY n.name, n.id LIMIT 1;
        IF v_node_id IS NULL THEN
            ...
            RETURN;
        END IF;
        UPDATE public.deployments SET step = 'handover', error = NULL, updated_at = now()
         WHERE id = v_dep.id;
        ...
        IF v_instance.id IS NULL THEN
            ...
            UPDATE public.deployments
               SET previous_image_ref = NULL,
                   ...
                   step = 'health', error = NULL, updated_at = now()
             WHERE id = v_dep.id;
        ELSE
    ...
    IF v_dep.step = 'health' THEN
        ...
        IF v_status.instance_id IS NOT NULL
           ...
           AND v_status.health = 'healthy' THEN
            UPDATE public.deployments
               SET status = 'succeeded', step = 'done',
        ...
        IF (
            ...
            IF v_dep.previous_image_ref IS NOT NULL AND v_instance.id IS NOT NULL THEN
                UPDATE public.instances
                   SET desired_state = 'running', image_ref = v_dep.previous_image_ref,
                       generation = generation + 1, updated_at = now()
                 WHERE id = v_instance.id;
                UPDATE public.deployments
                   SET status = 'rolled_back', step = 'done',
                       active_release_id = v_dep.previous_release_id,
                       error = v_reason, updated_at = now()
                 WHERE id = v_dep.id;
                INSERT INTO public.deployment_effects (
                    deployment_id, org_id, attempt_id, step, fencing_token
            ...
            IF v_instance.id IS NOT NULL THEN
                UPDATE public.instances
                   SET desired_state = 'stopped', generation = generation + 1, updated_at = now()
                 WHERE id = v_instance.id;
            END IF;
            UPDATE public.deployments
        ...
        UPDATE public.deployment_control_queue
           SET lease_expires_at = now() + make_interval(secs => p_lease_seconds),
               updated_at = now()
         WHERE deployment_id = v_dep.id AND lease_worker_id = p_worker_id;
        UPDATE public.deployment_leases
           SET expires_at = now() + make_interval(secs => p_lease_seconds), updated_at = now()
