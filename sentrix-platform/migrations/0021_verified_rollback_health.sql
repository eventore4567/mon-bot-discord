-- A rollback request is not proof that the previous image is serving traffic.
-- Keep the deployment running until the node-agent reports the restored generation
-- healthy. Only then expose `rolled_back` as a terminal status.
...
BEGIN
    ...
    IF v_dep.step = 'health' THEN
        ...
        IF (
            ...
            IF v_dep.previous_image_ref IS NOT NULL AND v_instance.id IS NOT NULL THEN
                ...
                ON CONFLICT DO NOTHING;
                RETURN QUERY
                    SELECT v_dep.id, 'running'::text, 'rollback_health'::text,
                           'rollback requested; waiting for healthy restored generation'::text;
                RETURN;
            END IF;
    ...
    IF v_dep.step = 'rollback_health' THEN
        SELECT i.* INTO v_instance
          FROM public.instances i
         WHERE i.env_id = v_dep.environment_id AND i.org_id = v_org_id;
        IF v_instance.id IS NOT NULL THEN
           ...
           AND v_status.health = 'healthy' THEN
            UPDATE public.deployments d
               SET status = 'rolled_back',
                   ...
                   health_deadline = NULL,
                   updated_at = now()
             WHERE d.id = v_dep.id;
            RETURN QUERY
                SELECT v_dep.id, 'rolled_back'::text, 'done'::text,
                       'rollback generation healthy'::text;
            RETURN;
        END IF;
        ...
        IF (
            ...
            v_reason := CASE
                WHEN v_dep.health_deadline IS NOT NULL AND v_dep.health_deadline <= now()
                THEN 'rollback health deadline exceeded'
                ELSE 'rollback generation reported unhealthy'
            END;
            IF v_instance.id IS NOT NULL THEN
                UPDATE public.instances i
                   SET desired_state = 'stopped',
            ...
            UPDATE public.deployments d
               SET status = 'failed',
                   step = 'done',
                   error = concat_ws('; ', NULLIF(d.error, ''), v_reason),
                   health_deadline = NULL,
                   updated_at = now()
             WHERE d.id = v_dep.id;
            RETURN QUERY
                SELECT v_dep.id, 'failed'::text, 'done'::text, v_reason;
            RETURN;
        END IF;
        ...
        RETURN QUERY
            SELECT v_dep.id, 'running'::text, 'rollback_health'::text,
                   'waiting for rollback health'::text;
        RETURN;
    END IF;
