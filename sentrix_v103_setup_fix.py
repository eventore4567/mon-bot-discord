...
V116 complète cette autorité avec un centre hiérarchique Modules -> Réglage. V103 reste
le dernier garde : il installe V116 après les couches V113/V114 puis republie une seule
commande slash /setup pointant vers Configuration._open_setup_panel.
"""
...
async def _send_setup_v114(bot, interaction: discord.Interaction) -> None:
    """Ouvre l'unique setup officiel : Configuration.SetupView patché V113/V114/V116."""
...
def _replace_setup_slash(bot) -> bool:
    """Réinstalle /setup avec une signature native, dirigée vers Configuration."""
    configuration = bot.get_cog("Configuration")
    ...
    setup_callback.__name__ = "slash_setup_v116"
    ...
    tree.add_command(
        app_commands.Command(
            name="setup",
            description="Configurer SentriX avec le centre de configuration",
...
def install() -> None:
    """Place V116 puis la garde /setup après toutes les transformations prepare_bot."""
    global _INSTALLED
    if _INSTALLED:
        return
    ...
    async def prepare_bot_v103(bot):
        ...
        try:
            from sentrix_setup_deep_v116 import install_for_bot as install_setup_v116
            install_setup_v116(bot)
        except Exception:
            logger.exception("V116 : installation du centre hiérarchique /setup impossible.")
    ...
    _INSTALLED = True
    logger.info("V103 setup authority guard armé pour Smart Setup V116.")
