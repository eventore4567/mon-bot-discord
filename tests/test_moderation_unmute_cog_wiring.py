"""cogs/moderation.py::unmute — Core V2, Phase 2 : câblage du corps de la
commande au-dessus de services/moderation.py::unmute(), déjà testé sans
Discord dans tests/test_services_moderation_unmute.py."""
...
class _FakeDB:
    def __init__(self, *, case_number=3):
...
class UnmuteCogWiringTests(unittest.IsolatedAsyncioTestCase):
    async def test_retrait_du_mute_reussi_appelle_le_service_et_rend_le_panneau(self):
        ...
        await Moderation.unmute.callback(cog, ctx, target, raison="comportement corrigé")
