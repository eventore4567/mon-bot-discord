...
const expectedTabs=["overview","welcome","levels","security","moderation","logs","verification","roles","economy","notifications","tickets","ai","embeds","config","access","diagnostic"];
...
const checks={
  ...
  access:"#commandList", diagnostic:".permission-grid",
};
for(const tab of expectedTabs){
  const button=dom.window.document.querySelector(`#navigation button[data-tab="${tab}"]`);
  button.click();
  await sleep(["overview","security","moderation","verification","economy","tickets","access","diagnostic"].includes(tab)?130:35);
...
for(const required of ["/api/guilds/1/diagnostics","/api/guilds/1/sanctions","/api/guilds/1/v62","/api/guilds/1/setup-tools"]){
...
paletteInput.value="sécurité";paletteInput.dispatchEvent(new dom.window.Event("input",{bubbles:true}));
if(!dom.window.document.querySelector('[data-palette-tab="security"]')) throw new Error("Recherche globale sécurité absente.");
