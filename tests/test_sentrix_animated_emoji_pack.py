...
def test_v36_runtime_never_adds_commands_or_deletes_server_emojis() -> None:
    ...
    assert "delete_custom_emoji" not in source
    assert ".delete(reason=" not in source
    assert "len(PACK)" in source


def test_v36_pack_has_exact_semantic_registry() -> None:
    source = RUNTIME.read_text(encoding="utf-8")
    for key in ("loading", "ok", "error", "no", "alert", "ticket", "staff", "update", "online"):
        assert f'"{key}": ("sxv36_{key}"' in source
