"""cogs/moderation.py::ban — Core V2, Phase 2 : le corps de la commande n'est plus
que l'adaptation Discord au-dessus de services/moderation.py::ban(), déjà testé
sans Discord dans tests/test_services_moderation_ban.py. Ces tests vérifient le
CÂBLAGE : le service est bien appelé, son résultat bien rendu, et le refus de
hiérarchie affiche bien le bon message sans jamais appeler guild.ban."""
...
class _FakeDB:
    def __init__(self, *, case_number=42):
...
class BanCogWiringTests(unittest.IsolatedAsyncioTestCase):
    async def test_bannissement_reussi_appelle_le_service_et_rend_le_panneau(self):
        ...
        await Moderation.ban.callback(cog, ctx, target, raison="spam")
    ...
    async def test_echec_de_persistance_n_empeche_pas_le_panneau_de_succes(self):
        ...
        service) doit rester silencieuse sur l'échec de persistance et afficher
        un vrai succès — c'est exactement le trou corrigé par cette migration."""
        cog = _make_cog()
