...
# Conservé uniquement pour compatibilité avec d'anciens appels de configuration. Le
# renderer officiel n'ajoute plus de bannière aux commandes ni aux logs.
SENTRIX_BANNER_URL = (
...
# ---------------------------------------------------------------------------
# CARTES DE COMMANDES — grandes, sobres, sans bannière
# ---------------------------------------------------------------------------
def standard(
    title: str,
    description: str = "",
...
def style_existing(
    ...
    *,
    ...
    """Normalise aussi les anciens embeds créés directement par les vieux cogs."""
    if not isinstance(embed, discord.Embed):
        return embed

    if embed.title:
        embed.title = clean_ui_text(embed.title, 256, "Information")
    else:
    ...
    for index, field in enumerate(list(embed.fields)):
        embed.set_field_at(
            index,
            name=clean_ui_text(field.name, 256, "Information"),
    ...
    if footer:
        embed.set_footer(text=clean_ui_text(footer, 2048, SENTRIX_FOOTER))
    else:
        embed.set_footer(text=SENTRIX_FOOTER)
...
def clean_view(view: Any) -> Any:
    """Retire les emojis décoratifs des composants sans toucher aux callbacks."""
    if view is None:
        return None
...
# ---------------------------------------------------------------------------
# JOURNAUX — même style, données métier conservées, aucune bannière
# ---------------------------------------------------------------------------
def log_embed(
    title: str,
    *,
...
def normalize_log(
    ...
    *,
    ...
    """Convertit un ancien embed métier en log SentriX large sans faux remplissage."""
    fields = []
...
# ---------------------------------------------------------------------------
# Filet de transport pour quelques journaux historiques envoyés directement
# ---------------------------------------------------------------------------
            ...
            if kwargs.get("view") is None and embed is not None:
                try:
                    ...
                    if derived is not None:
                        kwargs["view"] = clean_view(derived)
                except Exception:
                    pass
