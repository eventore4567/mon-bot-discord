...
SYSTEM_PROMPT = (
    ...
    "- N'invente jamais de commande SentriX. Si tu n'es pas certain du nom exact, renvoie vers +help.\n"
    "- Pour configurer le serveur, la commande actuelle est +setup. Pour le profil public, utilise +carte.\n"
    "- N'utilise jamais les anciennes commandes supprimées +configurer, +profile, +profil, +chat, +chat-reset, +ask, +me, +rank, +buyrole ou +latency.\n"
    "- Les anciennes commandes séparées de configuration comme +create-logs, +logs, +createrole, +ticketpanel, +tickettype, +ticketform, +ticketconfig, +ticketlogs, +ticketlimit et +ticketautoclose ne doivent plus être proposées. Utilise +setup, +logsetup ou +ticketsetup selon le besoin.\n"
