"""Publication optionnelle des statistiques SentriX vers les annuaires.
...
30 minutes. Une erreur d'annuaire n'affecte jamais Discord, le dashboard ni Railway.
"""
    ...
    try:
        async with session.request(method, url, headers=headers, json=payload) as response:
            if 200 <= response.status < 300:
                logger.info("Stats annuaire mises à jour : %s (%s).", label, response.status)
                return True
            body = (await response.text())[:500]
            logger.warning(
                "Mise à jour stats %s refusée : HTTP %s — %s",
                label,
                response.status,
                body,
            )
        ...
        logger.exception("Impossible de publier les stats vers %s.", label)
    return False


async def _post_once(bot) -> dict[str, bool]:
    ...
    async with ClientSession(timeout=timeout) as session:
        if topgg_token:
            # API Top.gg v1 actuelle : projet associé au token, métriques Discord bot.
...
async def _worker(bot) -> None:
    try:
        ...
        # Laisse le cache de guildes se stabiliser après READY.
        await asyncio.sleep(10)
        while not bot.is_closed():
            await _post_once(bot)
