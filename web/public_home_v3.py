...
body:before{content:"";position:fixed;inset:0;pointer-events:none;z-index:-5;background-image:
  ...
  background-size:72px 72px;mask-image:linear-gradient(to bottom,black 0 55%,transparent 92%)}
body:after{content:"";position:fixed;inset:0;pointer-events:none;z-index:-3;background:
  radial-gradient(560px circle at var(--mx) var(--my),rgba(124,108,255,.055),transparent 68%)}
canvas#fx{position:fixed;inset:0;z-index:-4;width:100%;height:100%;pointer-events:none;opacity:.28}
...
.noise{position:fixed;inset:0;pointer-events:none;z-index:120;opacity:.010;background-image:url("data:image/svg+xml,%3Csvg viewBox='0 0 180 180' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='n'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='.9' numOctaves='4' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23n)' opacity='.5'/%3E%3C/svg%3E")}

/* navigation */
.topbar{position:sticky;top:0;z-index:130;height:var(--nav);background:rgba(5,6,10,.64);border-bottom:1px solid rgba(255,255,255,.05);backdrop-filter:blur(14px) saturate(125%);transition:.25s ease}
...
.hero:before{content:"";position:absolute;left:50%;top:-260px;width:1100px;height:760px;transform:translateX(-50%);border-radius:50%;background:radial-gradient(circle,rgba(124,108,255,.105),rgba(76,169,255,.025) 42%,transparent 70%);filter:blur(28px);animation:heroBreath 12s ease-in-out infinite alternate}
...
.orbit{position:absolute;inset:28px 12px;border:1px solid rgba(124,108,255,.055);border-radius:50%;transform:rotate(-10deg);animation:orbit 22s linear infinite}.orbit:before,.orbit:after{content:"";position:absolute;width:8px;height:8px;border-radius:50%;background:var(--violet2);box-shadow:0 0 22px rgba(170,158,255,.85)}.orbit:before{left:13%;top:9%}.orbit:after{right:10%;bottom:14%;background:var(--blue)}
.product-shell{position:absolute;inset:42px 4px 36px 26px;overflow:hidden;border:1px solid rgba(255,255,255,.12);border-radius:26px;background:linear-gradient(145deg,rgba(17,22,32,.98),rgba(7,10,15,.99));box-shadow:0 34px 90px rgba(0,0,0,.48),0 0 48px rgba(124,108,255,.035);transform:rotateY(-5deg) rotateX(2deg);animation:floatPanel 6.8s ease-in-out infinite}
...
.float-card{position:absolute;z-index:5;padding:12px 13px;border:1px solid rgba(170,158,255,.14);border-radius:14px;background:rgba(13,18,26,.92);backdrop-filter:blur(10px);box-shadow:0 16px 38px rgba(0,0,0,.28);animation:floatCard 8s ease-in-out infinite}.float-card b{display:block;font-size:10px}.float-card span{display:block;margin-top:3px;color:var(--muted);font-size:8px}.float-a{right:-8px;top:82px}.float-b{display:none}.pulse-dots{display:flex;gap:5px;margin-top:8px}.pulse-dots i{width:5px;height:5px;border-radius:50%;background:var(--violet);animation:wave 1.35s ease-in-out infinite}.pulse-dots i:nth-child(2){animation-delay:.15s}.pulse-dots i:nth-child(3){animation-delay:.30s}
...
/* module rail */
.rail{overflow:hidden;margin:54px 0 0;border-top:1px solid rgba(255,255,255,.04);border-bottom:1px solid rgba(255,255,255,.04);background:rgba(255,255,255,.006)}.rail-track{display:flex;width:auto;justify-content:center;flex-wrap:wrap;gap:8px;padding:14px 20px;animation:none}.rail-track .chip:nth-child(n+13){display:none}.chip{display:inline-flex;align-items:center;gap:8px;padding:8px 12px;border:1px solid var(--line);border-radius:999px;background:#0c1118;color:var(--soft);font-size:9px;font-weight:850;white-space:nowrap}.chip i{width:6px;height:6px;border-radius:50%;background:var(--violet)}

/* sections */
.section{padding:128px 0}.section-head{display:flex;align-items:end;justify-content:space-between;gap:36px;margin-bottom:42px}.section-head>div{max-width:760px}.eyebrow{color:var(--violet2);font-size:10px;font-weight:950;letter-spacing:.12em;text-transform:uppercase}.section h2{margin:9px 0 0;font-size:clamp(33px,4.7vw,56px);line-height:1.03;letter-spacing:-.052em;text-wrap:balance}.section-head p{max-width:460px;margin:0;color:var(--muted);line-height:1.75}

/* Bento */
.bento{display:grid;grid-template-columns:repeat(12,1fr);grid-auto-rows:minmax(160px,auto);gap:18px}.card{position:relative;overflow:hidden;padding:23px;border:1px solid var(--line);border-radius:19px;background:rgba(13,18,26,.92);transition:transform .22s ease,border-color .22s ease,box-shadow .22s ease}.card:hover{border-color:#344155;box-shadow:0 18px 42px rgba(0,0,0,.22)}.card:after{display:none}.card.big{grid-column:span 6;grid-row:span 2}.card.med{grid-column:span 3}.card.wide{grid-column:span 6}.card.small{grid-column:span 3}.icon{width:44px;height:44px;border:1px solid var(--line2);border-radius:13px;background:#182131;display:grid;place-items:center;color:#cbc5ff;font-size:9px;font-weight:950;letter-spacing:.05em}.card h3{margin:18px 0 7px;font-size:18px}.card p{margin:0;color:var(--muted);font-size:12px;line-height:1.68}.card.big h3{font-size:23px}
...
if(!reduced){
  ...
  function aimTilt(el,x,y,scale=1){const r=el.getBoundingClientRect(),nx=Math.max(0,Math.min(1,(x-r.left)/r.width))-.5,ny=Math.max(0,Math.min(1,(y-r.top)/r.height))-.5,s=getMotion(el),max=el.classList.contains("card")?4.8:el.classList.contains("step")?4:3.2;s.trx=-ny*max*scale;s.try=nx*max*scale;s.ttx=nx*6*scale;s.tty=ny*4*scale;if(!s.raf)s.raf=requestAnimationFrame(()=>animateTilt(el))}
