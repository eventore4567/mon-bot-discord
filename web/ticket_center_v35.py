...
.sx-ticket-metric{padding:11px 12px;border:1px solid #252d40;border-radius:11px;background:#0f141e}
...
.sx-ticket-create{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:8px;padding:9px 12px;border-bottom:1px solid #222a3a}.sx-ticket-create button{min-height:36px;padding:0 12px;border:1px solid #725bd0;border-radius:9px;background:#765de0;color:#fff;font-weight:800;cursor:pointer}
...
.sx-ticket-state{display:inline-flex;align-items:center;justify-content:center;min-width:64px;padding:4px 7px;border:1px solid #343d52;border-radius:999px;background:#171d29;color:#b7bfd0;font-size:8px;font-weight:900;text-transform:uppercase}.sx-ticket-state.on{border-color:#3d6454;background:#15261f;color:#8ed8b4}.sx-ticket-state.warn{border-color:#6a5630;background:#281f11;color:#e4c06d}
.sx-ticket-actions{display:flex;justify-content:flex-end;gap:6px;flex-wrap:wrap}.sx-ticket-actions button,.sx-ticket-actions a{display:inline-flex;align-items:center;justify-content:center;min-height:31px;padding:0 9px;border:1px solid #343d52;border-radius:8px;background:#171d29;color:#dcd9e4;font-size:9px;font-weight:800;text-decoration:none;cursor:pointer}.sx-ticket-actions .primary{border-color:#6c55c8;background:#271f45;color:#cbbdff}.sx-ticket-actions .danger{border-color:#71343d;background:#2a171b;color:#f2a6ae}
...
.sx-ticket-editor{margin:10px 12px 12px;border:1px solid #353f56;border-radius:12px;background:#0b1019;overflow:hidden}.sx-ticket-editor-head{display:flex;justify-content:space-between;gap:12px;align-items:center;padding:12px;border-bottom:1px solid #252d3e}.sx-ticket-editor-head h4{margin:0;font-size:13px}.sx-ticket-editor-head p{margin:3px 0 0;color:#7f899c;font-size:9px}
.sx-editor-body{padding:12px;display:grid;gap:14px}.sx-editor-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:9px}.sx-editor-grid .full{grid-column:1/-1}.sx-editor-field label{display:block;margin:0 0 5px;color:#aeb6c6;font-size:9px;font-weight:800}.sx-editor-field input,.sx-editor-field textarea,.sx-editor-field select{width:100%;box-sizing:border-box;background:#111824;color:#eae8ef;border:1px solid #2c3548;border-radius:8px;padding:8px;font:inherit;font-size:10px}.sx-editor-field textarea{min-height:76px;resize:vertical}.sx-editor-field select[multiple]{min-height:110px}
.sx-editor-note{padding:8px 10px;border:1px solid #2b3448;border-radius:8px;background:#111722;color:#8993a6;font-size:9px;line-height:1.5}
.sx-type-wrap{display:grid;gap:9px}.sx-type-card{border:1px solid #293246;border-radius:10px;background:#0f151f;padding:10px;display:grid;gap:8px}.sx-type-title{display:flex;justify-content:space-between;gap:10px;align-items:center}.sx-type-title b{font-size:10px}.sx-mini-danger{border:1px solid #64343b;background:#26161a;color:#efa3ac;border-radius:7px;padding:5px 8px;font-size:8px;cursor:pointer}
.sx-editor-footer{display:flex;justify-content:flex-end;gap:8px;padding:11px 12px;border-top:1px solid #252d3e}.sx-editor-footer button{min-height:34px;padding:0 12px;border-radius:8px;border:1px solid #343d52;background:#171d29;color:#e5e2ea;font-weight:800;cursor:pointer}.sx-editor-footer .save{background:#6f57d4;border-color:#7e65e7;color:#fff}.sx-editor-footer .delete{margin-right:auto;background:#2b171b;border-color:#69343d;color:#f1a2aa}
...
@media(max-width:620px){.sx-ticket-metrics,.sx-editor-grid{grid-template-columns:1fr}.sx-editor-grid .full{grid-column:auto}.sx-ticket-row{grid-template-columns:minmax(0,1fr) auto}.sx-ticket-cell{display:none!important}.sx-ticket-create{grid-template-columns:1fr}.sx-ticket-create button{width:100%}}
</style>
"""
...
function roleOptions(selected=[],includeEmpty=false){const chosen=new Set((selected||[]).map(String));let h=includeEmpty?'<option value="">Aucun rôle de gestion</option>':"";return h+(data?.roles||[]).map(r=>`<option value="${esc(r.id)}" ${chosen.has(String(r.id))?"selected":""}>@${esc(r.name)}</option>`).join("")}
function selectedValues(el){return el?Array.from(el.selectedOptions).map(o=>o.value).filter(Boolean):[]}
function renderPanels(){const rows=data?.panels||[];if(!rows.length)return'<div class="sx-ticket-empty">Aucun panel. Créez votre premier panel ici ou avec +ticketsetup.</div>';return`<div class="sx-ticket-list">${rows.map(p=>`<div class="sx-ticket-row"><div class="sx-ticket-main"><b>${esc(p.name)}</b><span>${esc(p.title||"Sans titre")} · ${p.style==="button"?"Boutons":"Menu déroulant"}</span></div><span class="sx-ticket-cell">${p.channel_id?esc(channelName(p.channel_id)):"Pas encore envoyé"}</span><span class="sx-ticket-cell">${Number(p.type_count||0)} type(s) · max ${Number(p.max_per_member||1)}/membre</span><div class="sx-ticket-actions"><span class="sx-ticket-state ${p.enabled?"on":""}">${p.enabled?"Actif":"Inactif"}</span><button class="primary" data-panel-edit="${p.id}" type="button">Modifier</button><button data-panel-toggle="${p.id}" type="button">${p.enabled?"Désactiver":"Activer"}</button></div></div>`).join("")}</div>`}
function renderTypesSummary(){const rows=data?.types||[];if(!rows.length)return'<div class="sx-ticket-empty">Aucun type de ticket configuré.</div>';return`<div class="sx-ticket-list">${rows.map(t=>`<div class="sx-ticket-row"><div class="sx-ticket-main"><b>${esc(t.name)}</b><span>Panel #${esc(t.panel_id)}</span></div><span class="sx-ticket-cell">Bouton : ${esc(t.button_label||t.name)} · ${esc(t.button_style||"bleu")}</span><span class="sx-ticket-cell">Accès ${Number(t.role_rules?.access_role_ids?.length||0)} · Ping ${Number(t.role_rules?.ping_role_ids?.length||0)}</span><div class="sx-ticket-actions"><span class="sx-ticket-state ${t.role_rules?.configured?"on":""}">${t.role_rules?.configured?"Surcharge":"Hérite"}</span></div></div>`).join("")}</div>`}
function renderOpen(){const rows=data?.open_tickets||[];if(!rows.length)return'<div class="sx-ticket-empty">Aucun ticket ouvert actuellement.</div>';return`<div class="sx-ticket-list">${rows.map(t=>`<div class="sx-ticket-row"><div class="sx-ticket-main"><b>Ticket #${esc(t.id)}</b><span>${esc(channelName(t.channel_id))} · ouvert il y a ${esc(age(t.created_at))}</span></div><span class="sx-ticket-cell">Membre : ID ${esc(t.user_id)}</span><span class="sx-ticket-cell">${t.claimed_by?`Pris par ${esc(t.claimed_by)}`:"En attente"} · activité ${esc(age(t.last_activity_at))}</span><div class="sx-ticket-actions"><a href="https://discord.com/channels/${guildId()}/${t.channel_id}" target="_blank" rel="noopener">Ouvrir</a></div></div>`).join("")}</div>`}
function typeCard(t,i){const rr=t.role_rules||{configured:false,access_role_ids:[],ping_role_ids:[]};return`<div class="sx-type-card" data-type-index="${i}"><div class="sx-type-title"><b>Type / bouton ${i+1}${t.id?` · #${t.id}`:" · nouveau"}</b><button class="sx-mini-danger" data-remove-type="${i}" type="button">Supprimer</button></div><div class="sx-editor-grid"><div class="sx-editor-field"><label>Nom du type</label><input data-t="name" value="${esc(t.name||"Support")}" maxlength="80"></div><div class="sx-editor-field"><label>Nom du bouton</label><input data-t="button_label" value="${esc(t.button_label||"")}" maxlength="80"></div><div class="sx-editor-field"><label>Emoji</label><input data-t="emoji" value="${esc(t.emoji||"")}" maxlength="100"></div><div class="sx-editor-field"><label>Couleur du bouton</label><select data-t="button_style">${["bleu","gris","vert","rouge"].map(x=>`<option ${String(t.button_style||"bleu")===x?"selected":""}>${x}</option>`).join("")}</select></div><div class="sx-editor-field full"><label>Description courte</label><input data-t="description" value="${esc(t.description||"")}" maxlength="150"></div><div class="sx-editor-field full"><label>Message d'ouverture</label><textarea data-t="open_message" maxlength="1000">${esc(t.open_message||"")}</textarea></div><div class="sx-editor-field"><label>Rôle de gestion (boutons staff)</label><select data-t="staff_role_id">${roleOptions(t.staff_role_id?[t.staff_role_id]:[],true)}</select></div><div class="sx-editor-field"><label>Position</label><input data-t="position" type="number" min="0" max="24" value="${Number(t.position||i)}"></div><div class="sx-editor-field full"><label><input data-t="inherit_roles" type="checkbox" ${rr.configured?"":"checked"}> Hériter des rôles du panel</label></div><div class="sx-editor-field"><label>Rôles ayant accès au ticket</label><select data-t="access_role_ids" multiple>${roleOptions(rr.access_role_ids||[])}</select></div><div class="sx-editor-field"><label>Rôles ping à l'ouverture</label><select data-t="ping_role_ids" multiple>${roleOptions(rr.ping_role_ids||[])}</select></div></div></div>`}
function captureTypes(){document.querySelectorAll(".sx-type-card").forEach(card=>{const i=Number(card.dataset.typeIndex),t=editorTypes[i];if(!t)return;card.querySelectorAll("[data-t]").forEach(el=>{const k=el.dataset.t;if(k==="inherit_roles")t.inherit_roles=el.checked;else if(k==="access_role_ids"||k==="ping_role_ids")t[k]=selectedValues(el);else t[k]=el.value})})}
function renderEditor(){const p=data?.panels?.find(x=>String(x.id)===String(editingId));if(!p)return"";const rr=p.role_rules||{access_role_ids:[],ping_role_ids:[]};return`<div class="sx-ticket-editor"><div class="sx-ticket-editor-head"><div><h4>Modifier « ${esc(p.name)} »</h4><p>La sauvegarde met aussi à jour le message Discord déjà envoyé quand il existe.</p></div><button id="sxCloseEditor" class="sx-ticket-refresh" type="button">Fermer</button></div><div class="sx-editor-body"><div class="sx-editor-grid"><div class="sx-editor-field"><label>Nom interne</label><input id="sxPanelName" maxlength="80" value="${esc(p.name)}"></div><div class="sx-editor-field"><label>Titre affiché</label><input id="sxPanelTitle" maxlength="256" value="${esc(p.title||"")}"></div><div class="sx-editor-field full"><label>Description</label><textarea id="sxPanelDescription" maxlength="2000">${esc(p.description||"")}</textarea></div><div class="sx-editor-field"><label>Couleur hex</label><input id="sxPanelColor" maxlength="7" value="${p.color?("#"+Number(p.color).toString(16).padStart(6,"0")):""}"></div><div class="sx-editor-field"><label>Affichage des types</label><select id="sxPanelStyle"><option value="select" ${p.style!=="button"?"selected":""}>Menu déroulant</option><option value="button" ${p.style==="button"?"selected":""}>Boutons</option></select></div><div class="sx-editor-field full"><label>Image / bannière</label><input id="sxPanelImage" type="url" value="${esc(p.image_url||"")}"></div><div class="sx-editor-field"><label>Miniature</label><input id="sxPanelThumbnail" type="url" value="${esc(p.thumbnail_url||"")}"></div><div class="sx-editor-field"><label>Footer</label><input id="sxPanelFooter" maxlength="200" value="${esc(p.footer_text||"")}"></div><div class="sx-editor-field"><label>Tickets max / membre</label><input id="sxPanelMax" type="number" min="1" max="20" value="${Number(p.max_per_member||1)}"></div><div class="sx-editor-field"><label>État</label><select id="sxPanelEnabled"><option value="1" ${p.enabled?"selected":""}>Actif</option><option value="0" ${!p.enabled?"selected":""}>Inactif</option></select></div><div class="sx-editor-field"><label>Rôles ayant accès (sans être modérateur)</label><select id="sxPanelAccessRoles" multiple>${roleOptions(rr.access_role_ids||[])}</select></div><div class="sx-editor-field"><label>Rôles à ping</label><select id="sxPanelPingRoles" multiple>${roleOptions(rr.ping_role_ids||[])}</select></div></div><div class="sx-editor-note">Accès et ping sont séparés. Un rôle d'accès peut voir/répondre au ticket sans recevoir de permission de modération. Le rôle de gestion de chaque type contrôle les boutons staff.</div><div class="sx-type-title"><b>Boutons / types du panel</b><button id="sxAddType" class="sx-ticket-refresh" type="button">Ajouter un bouton</button></div><div class="sx-type-wrap">${editorTypes.map(typeCard).join("")||'<div class="sx-ticket-empty">Aucun type. Ajoutez au moins un bouton avant d’envoyer le panel.</div>'}</div></div><div class="sx-editor-footer"><button id="sxDeletePanel" class="delete" type="button">Supprimer le panel</button><button id="sxCancelEditor" type="button">Annuler</button><button id="sxSavePanel" class="save" type="button">Enregistrer et mettre à jour Discord</button></div></div>`}
function bindEditor(){if(!editingId)return;$("sxCloseEditor")?.addEventListener("click",closeEditor);$("sxCancelEditor")?.addEventListener("click",closeEditor);$("sxSavePanel")?.addEventListener("click",savePanel);$("sxDeletePanel")?.addEventListener("click",deletePanel);$("sxAddType")?.addEventListener("click",()=>{captureTypes();editorTypes.push({id:null,name:"Support",description:"",emoji:"",button_label:"Ouvrir un ticket",button_style:"bleu",open_message:"",staff_role_id:"",position:editorTypes.length,inherit_roles:true,access_role_ids:[],ping_role_ids:[],role_rules:{configured:false,access_role_ids:[],ping_role_ids:[]}});render()});document.querySelectorAll("[data-remove-type]").forEach(b=>b.addEventListener("click",()=>{captureTypes();const i=Number(b.dataset.removeType),t=editorTypes[i];if(t?.id)deletedTypeIds.push(Number(t.id));editorTypes.splice(i,1);render()}))}
...
function render(){const el=root();if(!el||!data)return;const m=data.metrics||{};el.innerHTML=`<div class="sx-ticket-head"><div><h3>Centre Tickets</h3><p>Sélectionnez un panel existant pour modifier son texte, son image, ses boutons et ses rôles.</p></div><button class="sx-ticket-refresh" id="sxTicketRefresh" type="button">Actualiser</button></div><div class="sx-ticket-metrics"><div class="sx-ticket-metric"><span>Ouverts</span><b>${Number(m.open||0)}</b></div><div class="sx-ticket-metric"><span>Non pris</span><b>${Number(m.unclaimed||0)}</b></div><div class="sx-ticket-metric"><span>Fermés / 7 jours</span><b>${Number(m.closed_7d||0)}</b></div><div class="sx-ticket-metric"><span>Note moyenne</span><b>${m.average_rating?Number(m.average_rating).toFixed(1)+" / 5":"—"}</b></div></div><section class="sx-ticket-section"><div class="sx-ticket-section-head"><b>Panels</b><span>${Number(data.panels?.length||0)} configuré(s)</span></div><div class="sx-ticket-create"><input id="sxTicketPanelName" maxlength="80" placeholder="Nom du nouveau panel"><button id="sxTicketCreatePanel" type="button">Créer</button></div>${renderPanels()}${editingId?renderEditor():""}</section><section class="sx-ticket-section"><div class="sx-ticket-section-head"><b>Types de tickets</b><span>${Number(data.types?.length||0)} type(s)</span></div>${renderTypesSummary()}</section><section class="sx-ticket-section"><div class="sx-ticket-section-head"><b>Tickets ouverts</b><span>${Number(data.open_tickets?.length||0)}</span></div>${renderOpen()}</section>`;
$("sxTicketRefresh")?.addEventListener("click",()=>load(guildId(),true));$("sxTicketCreatePanel")?.addEventListener("click",createPanel);el.querySelectorAll("[data-panel-toggle]").forEach(b=>b.addEventListener("click",()=>togglePanel(b.dataset.panelToggle)));el.querySelectorAll("[data-panel-edit]").forEach(b=>b.addEventListener("click",()=>openEditor(b.dataset.panelEdit)));bindEditor()}
...
def install(dashboard) -> None:
    ...
    logger.info("Dashboard V35 : éditeur complet de panels Tickets installé.")
