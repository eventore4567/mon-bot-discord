...
def _reponse(titre: str, description: str, *, kind: str = "brand") -> panels.Panneau:
    """Reponse SentriX Plus composee.

    Ce module repondait en texte brut ; il est passe a l'embed canonique, puis au
    panneau. Une description sur PLUSIEURS lignes devient une vraie section : ces
    reponses enumerent souvent ce qui vient de changer, et une enumeration en
    paragraphe se lit mal.
    """
    # Pas de section ici : une confirmation d'une ligne n'a rien a structurer.
    # Ce niveau est l'IDENTITE — banniere, accent, titre. Les ecrans qui ont
    # vraiment de la matiere sont composes a la main, la ou ils sont ecrits.
...
class SentriXPlus(commands.Cog, name="SentriXPlus"):
    ...
    async def starboard_off(self, ctx: commands.Context):
        ...
        await panels.envoyer(ctx, _reponse("Starboard", 'Starboard désactivé sur ce serveur.', kind="success"))
    ...
    async def voicehub_setup(self, ctx: commands.Context):
        ...
        if me is None or not me.guild_permissions.manage_channels or not me.guild_permissions.move_members:
            return await panels.envoyer(ctx, _reponse("VoiceHub", 'SentriX a besoin de Gérer les salons et Déplacer des membres pour activer VoiceHub.', kind="danger"))
    ...
    async def voicehub_off(self, ctx: commands.Context):
        ...
        await panels.envoyer(ctx, _reponse("VoiceHub", "VoiceHub désactivé. Les vocaux temporaires existants restent actifs jusqu'à ce qu'ils soient vides.", kind="success"))
    ...
    async def voice_name(self, ctx: commands.Context, *, name: str):
        ...
        if channel is None or row is None:
            return await panels.envoyer(ctx, _reponse("Salon vocal", "Vous devez être propriétaire d'un vocal temporaire SentriX.", kind="danger"))
        name = str(name).strip()[:90]
        if len(name) < 2:
            return await panels.envoyer(ctx, _reponse("Salon vocal", 'Choisissez un nom de salon plus long.', kind="danger"))
    ...
    async def voice_limit(self, ctx: commands.Context, limit: int):
        ...
        if limit < 0 or limit > 99:
            return await panels.envoyer(ctx, _reponse("Salon vocal", 'La limite doit être comprise entre 0 et 99. 0 signifie illimité.', kind="danger"))
    ...
    async def voice_transfer(self, ctx: commands.Context, member: discord.Member):
        ...
        if member.bot or member not in channel.members:
            return await panels.envoyer(ctx, _reponse("Salon vocal", 'Le nouveau propriétaire doit être un membre présent dans votre vocal.', kind="danger"))
        await self.bot.db.execute(
        ...
        # proprietaire. Un message Components V2 n'accepte pas de `content` : la
        # mention vit donc dans le panneau, et allowed_mentions decide seul si elle
        # notifie — ouvert sur ce membre, ferme sur @everyone et les roles.
        await panels.envoyer(
            ctx,
            panels.Panneau(
                titre="SentriX — Salon vocal",
                sous_titre=f"{member.mention} est maintenant propriétaire de ce vocal.",
                kind="success",
                sections=[
                    panels.Section(
                        "Ce qui change",
                        [
                            panels.Ligne("Nouveau propriétaire", member.mention),
                            panels.Ligne(
                                "Il peut désormais",
                                "Renommer, limiter, verrouiller le salon",
                                indice="`+voice-name`, `+voice-limit`, `+voice-lock`.",
                            ),
                        ],
                    )
                ],
    ...
    async def sticky_set(self, ctx: commands.Context, channel: discord.TextChannel, *, message: str):
        ...
        if not content or len(content) > 1700:
            return await panels.envoyer(ctx, _reponse("Message sticky", 'Le message sticky doit contenir entre 1 et 1700 caractères.', kind="danger"))
    ...
    async def sticky_every(self, ctx: commands.Context, channel: discord.TextChannel, every: int):
        ...
        if not row:
            return await panels.envoyer(ctx, _reponse("Message sticky", "Aucun sticky n'est configuré dans ce salon.", kind="warning"))
        await self.bot.db.execute(
    ...
    async def sticky_off(self, ctx: commands.Context, channel: discord.TextChannel):
        ...
        await panels.envoyer(ctx, _reponse("Message sticky", f'Sticky désactivé dans {channel.mention}.', kind="success"))
    ...
    async def schedule_send(self, ctx: commands.Context, delay: str, channel: discord.TextChannel, *, message: str):
        ...
        if seconds is None:
            return await panels.envoyer(ctx, _reponse("Annonce programmée", 'Durée invalide. Exemples : 10m, 2h, 1d. Minimum 1 minute, maximum 30 jours.', kind="danger"))
        content = str(message).strip()
        if not content or len(content) > 1900:
            return await panels.envoyer(ctx, _reponse("Annonce programmée", 'Le message planifié doit contenir entre 1 et 1900 caractères.', kind="danger"))
    ...
    async def schedule_list(self, ctx: commands.Context):
        ...
        if not rows:
            return await panels.envoyer(ctx, _reponse("Annonces programmées", 'Aucune annonce programmée.', kind="warning"))
        now_ts = int(time.time())
        lines = []
        for row in rows:
    ...
    async def schedule_cancel(self, ctx: commands.Context, ident: int):
        ...
        if not row:
            return await panels.envoyer(ctx, _reponse("Annonce programmée", 'Annonce programmée introuvable.', kind="warning"))
        await self.bot.db.execute(
            "DELETE FROM sentrix_scheduled_messages WHERE id=? AND guild_id=?",
            (ident, ctx.guild.id),
        )
        await panels.envoyer(ctx, _reponse("Annonce programmée", f'Annonce #{ident} annulée.', kind="success"))
    ...
    async def server_health(self, ctx: commands.Context):
        ...
        me = guild.me
        if me is None:
            return await panels.envoyer(ctx, _reponse("Diagnostic du serveur", 'Impossible de lire les permissions de SentriX sur ce serveur.', kind="danger"))
        ...
        # Le diagnostic tenait en huit lignes de meme poids. Ce qui MANQUE passe
        # devant, et la couleur suit le score plutot que d'etre fixe.
        intention = "success" if not missing else "warning" if score >= 60 else "danger"
        sections = []
        if missing:
            sections.append(
                panels.Section(
                    f"Permissions manquantes ({len(missing)})",
                    [
                        panels.Ligne("SentriX ne peut pas", " · ".join(missing[:6]),
        ...
        sections.append(
            panels.Section(
                "Santé",
                [
                    panels.Ligne("Score", f"**{score} %**"),
                    panels.Ligne("Permissions", f"{passed} sur {total}"),
                    panels.Ligne("Vérification Discord", security),
                ],
                aligne=True,
            )
        )
        sections.append(
            panels.Section(
                "Fonctionnalités SentriX Plus",
                [
                    panels.Ligne("Starboard", "actif" if starboard else "non configuré"),
                    panels.Ligne("VoiceHub", "actif" if voicehub else "non configuré"),
                    panels.Ligne("Messages sticky", str(sticky_count)),
                    panels.Ligne("Annonces programmées", str(scheduled_count)),
                ],
                aligne=True,
            )
        )
        await panels.envoyer(
            ctx,
            panels.Panneau(
                titre="SentriX — Diagnostic du serveur",
                sous_titre=f"**{score} %** des permissions essentielles sont accordées sur {guild.name}.",
                kind=intention,
                sections=sections,
                pied="SentriX • Plus",
            ),
    ...
    async def sentrix_plus(self, ctx: commands.Context):
        await panels.envoyer(
            ...
            panels.Panneau(
                titre="SentriX — Plus",
                sous_titre="Les fonctionnalités ajoutées par SentriX Plus sur ce serveur.",
                kind="brand",
                sections=[
                    panels.Section(
                        "Mise en avant",
                        [panels.Ligne("`+starboard-setup #salon 3`",
                                      "Épingle automatiquement les messages les plus réagis")],
                    ),
                    panels.Section(
                        "Vocaux temporaires",
                        [
                            panels.Ligne("`+voicehub-setup`", "Crée le salon d'accueil"),
                            panels.Ligne("Puis", "`+voice-name` · `+voice-limit` · `+voice-lock`"),
                        ],
                    ),
                    panels.Section(
                        "Messages",
                        [
                            panels.Ligne("`+sticky-set #salon <texte>`", "Garde un message en bas du salon"),
                            panels.Ligne("`+schedule-send 2h #salon <texte>`", "Publie plus tard"),
                        ],
                    ),
                    panels.Section(
                        "Vérifier",
                        [panels.Ligne("`+server-health`", "Diagnostic des permissions et des modules")],
                    ),
                ],
                pied="SentriX • Plus",
            ),
