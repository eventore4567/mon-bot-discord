...
class Music(commands.Cog, name="Music"):
    ...
    async def join(self, ctx: commands.Context):
        if not ctx.author.voice:
            return await panels.envoyer(ctx, panels.depuis_embed(await self._embed(ctx.guild.id, title='Salon vocal requis', description='Vous devez être dans un salon vocal.', kind='danger')))
        ...
        await panels.envoyer(ctx, panels.depuis_embed(await self._embed(ctx.guild.id, title='Salon rejoint', description=f"J'ai rejoint **{channel.name}**.", kind='success')))
    ...
    async def play(self, ctx: commands.Context, *, recherche: str):
        ...
        if not state.voice_client.is_playing() and not state.current:
            ...
            await panels.envoyer(ctx, panels.depuis_embed(await self._embed(ctx.guild.id, title='Lecture en cours', description=f"▶️ Lecture de **{track['title']}**", kind='success')))
        else:
            await panels.envoyer(ctx, panels.depuis_embed(await self._embed(ctx.guild.id, title='Ajouté à la file', description=f"➕ **{track['title']}** ajouté à la file d'attente.", kind='success')))
    ...
    async def loop(self, ctx: commands.Context):
        ...
        await panels.envoyer(ctx, panels.depuis_embed(await self._embed(ctx.guild.id, title='Répétition', description=f"Répétition {('activée' if state.loop else 'désactivée')}.", kind='success')))
    ...
    async def remove_from_queue(self, ctx: commands.Context, position: int):
        ...
        await panels.envoyer(ctx, panels.depuis_embed(await self._embed(ctx.guild.id, title='Retiré de la file', description=f"🗑️ **{removed['title']}** retiré de la file d'attente.", kind='success')))
    ...
    async def playlist_save(self, ctx: commands.Context, *, nom: str):
        ...
        state = self.get_state(ctx.guild.id)
        if not state.queue:
            return await panels.envoyer(ctx, panels.depuis_embed(await self._embed(ctx.guild.id, title="File d'attente vide", kind='danger')))
        await self.bot.db.execute(
            "INSERT INTO playlists (guild_id, user_id, name, tracks) VALUES (?, ?, ?, ?)",
            (ctx.guild.id, ctx.author.id, nom, json.dumps(state.queue)),
        )
        await panels.envoyer(ctx, panels.depuis_embed(await self._embed(ctx.guild.id, title='Playlist sauvegardée', description=f'💾 Playlist **{nom}** sauvegardée ({len(state.queue)} titres).', kind='success')))
    ...
    async def playlist_load(self, ctx: commands.Context, *, nom: str):
        ...
        if not row:
            return await panels.envoyer(ctx, panels.depuis_embed(await self._embed(ctx.guild.id, title='Playlist introuvable', kind='danger')))
        tracks = json.loads(row["tracks"])
        state = self.get_state(ctx.guild.id)
        state.queue.extend(tracks)
        await panels.envoyer(ctx, panels.depuis_embed(await self._embed(ctx.guild.id, title='Playlist chargée', description=f'📂 Playlist **{nom}** chargée ({len(tracks)} titres ajoutés).', kind='success')))
