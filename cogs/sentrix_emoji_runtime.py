"""SentriX V3.7 — pack d'emojis animés compact et lisible.

Le pack contient dix GIFs 64x64. Chaque GIF est UN emoji Discord animé composé
de plusieurs frames. Au démarrage, SentriX retire uniquement son ancien pack
`sxv36_*`, synchronise le pack `sxv37_*` sur le serveur officiel puis réutilise les
IDs dans l'interface.
...
def _semantic_key(text: Any, *, category: str | None = None) -> str | None:
    haystack = f" {str(text or '').casefold()} "
    category = str(category or "").casefold()
    if any(word in haystack for word in ("ticket", "support")) or category == "tickets": return "ticket"
    if any(word in haystack for word in ("sécurité", "securite", "security", "modération", "moderation", "staff", "automod")): return "staff"
    if category in {"security", "moderation"}: return "staff"
    if any(word in haystack for word in ("actualiser", "actualisation", "refresh", "mise à jour", "mise a jour", "update")): return "update"
    if any(word in haystack for word in ("en direct", "online", "statut", "status", "connecté", "connecte")): return "online"
    if any(word in haystack for word in ("premium", "vip", "boost")) or category == "premium": return "premium"
    return None


def _static_icon(text: Any, *, category: str | None = None) -> str:
    semantic = _semantic_key(text, category=category)
    if semantic:
        animated = emoji_text(semantic)
        if animated: return animated
    haystack = f" {str(text or '').casefold()} "
    for icon, words in STATIC_RULES:
        if any(word in haystack for word in words): return icon
...
def _animated_state_key(text: Any, *, kind: str | None = None) -> str | None:
    ...
    if any(word in haystack for word in ("chargement", "loading", "patiente", "génération", "generation")): return "loading"
    if any(word in haystack for word in ("mise à jour", "mise a jour", "update en cours", "actualisation en cours")): return "update"
    if any(word in haystack for word in ("bot en ligne", "online", "connecté", "connecte")): return "online"
    if any(word in haystack for word in ("refusé", "refuse", "interdit", "denied")): return "no"
    return None
...
def _decorate_label(value: Any, *, category: str | None = None, kind: str | None = None) -> str:
    ...
    if state_key:
        ...
        if animated: return f"{animated} {text}".strip()
...
def _decorate_embed(embed: discord.Embed, *, command: Any = None, category: str | None = None, kind: str | None = None, log_type: str | None = None) -> discord.Embed:
    if log_type: return embed
    resolved_category = premium_style.infer_category(command=command, embed=embed, hint=category)
    if resolved_category == "logs": return embed
    ...
    for index, field in enumerate(list(embed.fields)):
        ...
        embed.set_field_at(index, name=f"{icon} {clean_name or 'Information'}"[:256], value=clean_value, inline=bool(field.inline))
    return embed


def _button_animated_key(text: str) -> str | None:
    haystack = str(text or "").casefold()
    if any(word in haystack for word in ("ticket", "support")): return "ticket"
    if any(word in haystack for word in ("sécurité", "securite", "security", "modération", "moderation", "automod", "staff")): return "staff"
    if any(word in haystack for word in ("actualiser", "refresh", "update")): return "update"
    if any(word in haystack for word in ("premium", "vip", "boost")): return "premium"
    if any(word in haystack for word in ("valider", "confirm", "confirmer", "enregistrer", "save")): return "ok"
    if any(word in haystack for word in ("fermer", "close", "annuler", "cancel")): return "no"
    return None
...
def _button_icon(text: str) -> str | discord.Emoji | None:
    ...
    if key:
        animated = emoji(key)
        if animated is not None: return animated
...
def _decorate_view(view: discord.ui.View | None) -> discord.ui.View | None:
    ...
    for item in list(getattr(view, "children", ()) or ()):
        if isinstance(item, discord.ui.Button):
            ...
            if item.label is not None: item.label = (label or "Action")[:80]
...
async def _read_setting(bot: commands.Bot, key: str) -> str | None:
    try: row = await bot.db.fetchone("SELECT value FROM bot_settings WHERE key = ?", (key,))
    except Exception: return None
    if not row: return None
    try: return str(row["value"])
    except Exception:
        try: return str(row[0])
        except Exception: return None
...
async def _resolve_official_guild(bot: commands.Bot) -> discord.Guild | None:
    ...
    if runtime is not None and callable(getattr(runtime, "official_guild_id", None)):
        ...
        except Exception: logger.debug("Résolution serveur officiel via runtime impossible.", exc_info=True)
    for setting in ("sentrix_official_guild_id", "sentrix_release_announce_guild_id"):
        raw = await _read_setting(bot, setting)
        if not raw: continue
        try: guild = bot.get_guild(int(raw))
        except ValueError: guild = None
        if guild is not None: return guild
    for guild in bot.guilds:
        names = {str(channel.name).casefold() for channel in guild.text_channels}
        if any("annonces-sentrix" in name for name in names) and any("règlement" in name or "reglement" in name for name in names): return guild
    try:
...
def _refresh_registry(emojis: list[discord.Emoji] | tuple[discord.Emoji, ...]) -> int:
    ...
    for key, (name, _filename) in PACK.items():
        ...
        if value is not None: _REGISTRY[key] = value
...
async def _delete_legacy_pack(guild: discord.Guild, current: list[discord.Emoji]) -> list[discord.Emoji]:
    ...
    if not legacy: return current
    if not _can_manage_expressions(guild):
        logger.warning("Pack SentriX : anciens sxv36_* présents mais permission insuffisante.")
        return current
    deleted_ids: set[int] = set()
    for value in legacy:
        try: await value.delete(reason="SentriX V3.7 — remplacement du pack animé")
        except discord.Forbidden:
            logger.warning("Pack SentriX : suppression de %s refusée.", value.name); break
        except discord.HTTPException as exc:
            logger.warning("Pack SentriX : suppression de %s impossible : %s", value.name, exc); continue
        deleted_ids.add(value.id)
    if deleted_ids: logger.info("Pack SentriX : %s ancien(s) sxv36_* supprimé(s).", len(deleted_ids))
    return [value for value in current if value.id not in deleted_ids]


async def sync_pack(bot: commands.Bot) -> int:
    """Remplace l'ancien pack et synchronise les dix emojis V3.7 sur le serveur officiel."""
    async with _SYNC_LOCK:
        guild = await _resolve_official_guild(bot)
        if guild is None:
            logger.warning("Pack SentriX : serveur officiel introuvable ; synchronisation reportée."); return 0
        try: current = list(await guild.fetch_emojis())
        except (discord.Forbidden, discord.HTTPException): current = list(guild.emojis)
        current = await _delete_legacy_pack(guild, current)
        count = _refresh_registry(current)
        if count == len(PACK):
            bot._sentrix_animated_emojis_ready = True; bot._sentrix_emoji_registry = dict(_REGISTRY); return count
        if not _can_manage_expressions(guild):
            logger.warning("Pack SentriX : permission Gérer les expressions manquante sur %s (%s).", guild.name, guild.id)
            bot._sentrix_emoji_registry = dict(_REGISTRY); return count
        existing_names = {value.name for value in current}
        for key, (name, filename) in PACK.items():
            if key in _REGISTRY or name in existing_names: continue
            path = ASSET_DIR / filename
            try: data = path.read_bytes()
            except OSError:
                logger.error("Pack SentriX : asset introuvable %s.", path); continue
            try: created = await guild.create_custom_emoji(name=name, image=data, reason="Pack UI SentriX V3.7")
            except discord.Forbidden:
                logger.warning("Pack SentriX : Discord refuse la création de %s.", name); break
            except discord.HTTPException as exc:
                logger.warning("Pack SentriX : création de %s impossible : %s", name, exc); continue
            _REGISTRY[key] = created; existing_names.add(name)
        bot._sentrix_emoji_registry = dict(_REGISTRY)
        bot._sentrix_animated_emojis_ready = len(_REGISTRY) == len(PACK)
        logger.info("Pack SentriX V3.7 synchronisé : %s/%s sur %s.", len(_REGISTRY), len(PACK), guild.name)
        return len(_REGISTRY)


def _install_sync(bot: commands.Bot) -> None:
    if getattr(bot, "_sentrix_emoji_sync_installed", False): return
    async def sync_on_ready() -> None:
        try: await sync_pack(bot)
        except Exception: logger.exception("Synchronisation du pack emojis SentriX impossible.")
    async def refresh_on_emoji_update(guild: discord.Guild, before: tuple[discord.Emoji, ...], after: tuple[discord.Emoji, ...]) -> None:
        ...
        if official is None or guild.id != official.id: return
        _refresh_registry(after); bot._sentrix_emoji_registry = dict(_REGISTRY); bot._sentrix_animated_emojis_ready = len(_REGISTRY) == len(PACK)
...
def _install_visual_pipeline() -> None:
    ...
    def style_embed_v37(embed: discord.Embed, *args, **kwargs):
        result = original_embed(embed, *args, **kwargs)
        if not isinstance(result, discord.Embed): return result
        return _decorate_embed(result, command=kwargs.get("command"), category=kwargs.get("category"), kind=kwargs.get("kind"), log_type=kwargs.get("log_type"))
    def style_view_v37(view: discord.ui.View | None): return _decorate_view(original_view(view))
    premium_style.style_embed = style_embed_v37
    premium_style.style_view = style_view_v37
    _INSTALLED = True


def install(bot: commands.Bot) -> None:
    _install_visual_pipeline(); _install_sync(bot)
    bot._sentrix_animated_emoji_pack_v37 = True
    bot._sentrix_clear_ui_emojis_v362 = True
    logger.info("SentriX V3.7 : pack animé compact actif ; migration sxv36 -> sxv37 programmée.")


__all__ = ["PACK", "LEGACY_PACK_PREFIXES", "CATEGORY_ICONS", "emoji", "emoji_text", "install", "sync_pack", "_clean_artifacts", "_decorate_embed", "_decorate_view", "_is_legacy_pack_emoji_name"]
