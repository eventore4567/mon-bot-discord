...
class AutomaticVerificationV5(v4.AutomaticVerification, name=_COG_NAME):
    ...
    async def start_human_verification(self, interaction: discord.Interaction):
        """Compatibilité du bouton persistant avec le moteur adaptatif V5."""
        if interaction.guild is None or not isinstance(interaction.user, discord.Member):
            embed = discord.Embed(title="SentriX — Vérification", description="Cette vérification fonctionne uniquement dans un serveur.", colour=discord.Colour.red())
        ...
        if bool(getattr(member, "pending", False)):
            embed = discord.Embed(title="Règles Discord requises", description="Accepte d'abord les règles du serveur Discord, puis clique de nouveau sur **Commencer**.", colour=discord.Colour.orange())
            return await panels.envoyer(interaction.response, panels.depuis_embed(embed), ephemere=True)
        result = await self.evaluate_member(member, reason="manual-button")
        if result is None:
            embed = discord.Embed(title="Vérification indisponible", description="La vérification n'est pas activée ou ne peut pas être exécutée pour ce compte.", colour=discord.Colour.orange())
            return await panels.envoyer(interaction.response, panels.depuis_embed(embed), ephemere=True)
        score, passed = result
        settings = await self.settings(member.guild.id)
        ...
        if passed:
            ...
            description = f"Ton accès est validé. Score de confiance : **{score:.2f}/{FACTOR_COUNT}** (seuil **{threshold}/{FACTOR_COUNT}**)."
            colour = discord.Colour.green()
        elif score >= threshold - BORDERLINE_MARGIN:
            title = "Second contrôle en cours"
            description = f"Score actuel : **{score:.2f}/{FACTOR_COUNT}**. Le compte est proche du seuil et SentriX lancera automatiquement un second contrôle."
            colour = discord.Colour.orange()
        else:
            title = "Vérification non validée"
            description = f"Score actuel : **{score:.2f}/{FACTOR_COUNT}** pour un seuil de **{threshold}/{FACTOR_COUNT}**. Aucun bannissement automatique : le compte reste en attente pour une nouvelle vérification ou une revue staff."
            colour = discord.Colour.orange()
