...
STAFF_COGS = {"Moderation", "Automod", "Security", "Configuration", "Logs", "ServerBuilder", "Verification", "Owner", "EmbedBuilder"}
...
def _category(command: commands.Command) -> str:
    cog_name = getattr(command.cog, "qualified_name", "Utility") if command.cog else "Utility"
    return CATEGORY_NAMES.get(cog_name, cog_name)
...
def _visible(bot: commands.Bot, member) -> list[commands.Command]:
    ...
    for command in bot.walk_commands():
        if command.hidden:
            continue
        cog_name = getattr(command.cog, "qualified_name", "Utility") if command.cog else "Utility"
        if not staff and cog_name in STAFF_COGS:
            continue
        rows.append(command)
    return rows
...
def _description(command: commands.Command) -> str:
    return (command.description or command.help or "Aucune description.").strip().split("\n", 1)[0][:180]
...
def _search(commands_list: list[commands.Command], query: str) -> list[commands.Command]:
    ...
    for command in commands_list:
        ...
        haystack = f"{command.qualified_name} {aliases} {_description(command)} {_category(command)}".casefold()
...
def _detail(bot: commands.Bot, command: commands.Command, prefix: str) -> discord.Embed:
    ...
    if command.aliases:
        panel.add_field(name="Alias", value=", ".join(f"`{alias}`" for alias in command.aliases[:8]), inline=True)
    panel.add_field(name="Catégorie", value=_category(command), inline=True)
    return panel


def _pages(bot: commands.Bot, commands_list: list[commands.Command], prefix: str, title: str) -> list[discord.Embed]:
...
class SearchModal(discord.ui.Modal, title="Rechercher une commande"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        view = HelpView(self.help_view.bot, self.help_view.prefix, interaction.user.id, pages=pages)
...
class CategorySelect(discord.ui.Select):
    def __init__(self, owner: "HelpView"):
        ...
        for command in _visible(owner.bot, owner.member):
            name = _category(command)
            grouped[name] = grouped.get(name, 0) + 1
        options = [discord.SelectOption(label=name[:100], value=name, description=f"{count} commande(s)") for name, count in grouped.items()]
        super().__init__(placeholder="Sélectionnez une catégorie", options=options[:25], row=0)

    async def callback(self, interaction: discord.Interaction):
        if interaction.user.id != self.owner.author_id:
            return await interaction.response.send_message("Ce panneau appartient à une autre personne.", ephemeral=True)
        ...
        view = HelpView(self.owner.bot, self.owner.prefix, interaction.user.id, pages=pages)
        await interaction.response.edit_message(embed=pages[0], view=view)


class HelpView(discord.ui.View):
    def __init__(self, bot: commands.Bot, prefix: str, author_id: int, *, pages: list[discord.Embed] | None = None, member=None):
        ...
        if dashboard_url.startswith("https://"):
            self.add_item(discord.ui.Button(label="Dashboard", style=discord.ButtonStyle.secondary, url=dashboard_url, row=1))
        self._sync()
...
class OfficialHelp(commands.Cog, name="SentriXHelp"):
    ...
    async def send_help(self, target, query: str | None = None):
        ...
        if query:
            ...
            exact = next((command for command in rows if query.casefold() in {command.name.casefold(), command.qualified_name.casefold(), *(alias.casefold() for alias in command.aliases)}), None)
            if exact:
...
async def setup(bot: commands.Bot):
    # Déconnecte réellement les anciens handlers avant d'enregistrer le propriétaire officiel.
