...
from .ticket_ping_role import install as install_ticket_ping_role
...
async def _load_extension_with_sentrix_patches(
    ...
    *,
    ...
    if name == "cogs.configuration" or name.endswith(".configuration"):
        ...
        # Ajoute le choix du rôle à ping directement dans la page Tickets de +setup.
        await install_ticket_ping_role(bot)
    if name == "cogs.tickets" or name.endswith(".tickets"):
        install_ticket_claim_security(bot)
        # Sépare le rôle mentionné à l'ouverture du rôle staff qui voit le salon.
        await install_ticket_ping_role(bot)
