"""Initialisation canonique des correctifs runtime SentriX.

Les correctifs propres à un Cog sont installés lors de son chargement. Les politiques
transversales (style, aide, erreurs, permissions, registre slash) sont finalisées UNE fois
après la dernière extension. Les anciens moteurs d'aide V3/V8 et les coordinateurs V2 ne
font plus partie du chemin actif.
"""
...
async def _install_configuration_critical_patches(bot: commands.Bot) -> None:
    ...
    await _run_installer("setup canonique", install_setup_oxyde_style, bot)
...
async def _install_common_runtime(bot: commands.Bot) -> None:
    ...
    await _run_installer("branding premium", install_premium_style, bot)
...
async def _install_extension_specific(bot: commands.Bot, name: str) -> None:
    ...
    if _matches(name, "cogs.utility"):
        ...
        # help_complete reste le catalogue de métadonnées/catégories. Son rendu historique
        # n'est plus final : help_simple en prend l'unique propriété à la fin du bootstrap.
        await _run_installer("catalogue aide", install_complete_help, bot)
        await _run_installer("accès dashboard", install_dashboard_access, bot)
...
async def _install_help_and_error_stack(bot: commands.Bot) -> None:
    """Aide simple unique + gestionnaire d'erreurs unique."""
    # final_runtime_polish installe easy_command_surface puis help_simple. Aucun ancien
    # renderer d'aide (help_clean_style, sentrix_v3_ux, help_home_circles) n'est exécuté.
    await _run_installer("surface et aide finales", install_final_runtime_polish, bot)
    ...
    try:
        ...
        logger.exception("Impossible de neutraliser le cooldown de l'aide.")
...
async def finalize_runtime(bot: commands.Bot) -> None:
    ...
    try:
        ...
        await _run_installer("compatibilité style global", install_global_style, bot)
    except Exception:
        logger.exception("Impossible de préparer la compatibilité de style SentriX.")
    ...
    logger.info("Runtime SentriX finalisé : une aide, un setup, un style, un transport, une politique d'erreurs.")
