...
class Security(commands.Cog):
    ...
    async def permission_audit(self, ctx: commands.Context):
        ...
        e = embeds.neutral("🔍 Audit des permissions" if not findings else "🔍 Audit des permissions — points relevés")
        if not findings:
            e.description = "✅ Aucun problème de permission évident détecté. Bon travail !"
            e.color = 0x57F287
        else:
            for label, text in findings:
                e.add_field(name=label, value=helpers.truncate(text, 1024), inline=False)
        e.set_footer(text=f"SentriX • {len(findings)} point(s) relevé(s) sur {len(guild.roles)} rôles")
        await ctx.send(embed=e)
